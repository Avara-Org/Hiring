module base_module::pay_stream_tests {
    #[test_only]
    use std::string;
    #[test_only]
    ...
    #[test_only]
    use base_module::pay_stream::{create_payment, accept_payment, cancel_payment, claim_payment, get_payment};
    ...
    public entry fun test_create_and_cancel(aptos_framework: &signer, sender: &signer, receiver: &signer) {
        ...
        let test_amount = 10;
        let test_period = 10;
        create_payment(sender, receiver_address, test_amount, test_period);

        let (period_in_seconds, start_time, value) = get_payment(sender_address, receiver_address);
        ...
        cancel_payment(sender, sender_address, receiver_address);
        ...
        create_payment(sender, receiver_address, test_amount * 2, test_period * 2);
        let (period_in_seconds, start_time, value) = get_payment(sender_address, receiver_address);
        assert!(period_in_seconds == test_period * 2, 0);
        assert!(start_time == 0, 0);
        assert!(value == test_amount * 2, 0);
        cancel_payment(receiver, sender_address, receiver_address);
    ...
    public entry fun test_create_accept_and_claim(aptos_framework: &signer, sender: &signer, receiver: &signer) {
        ...
        create_payment(sender, receiver_address, amount, period_in_seconds);
        let now = 1;
        timestamp::fast_forward_seconds(now);
        accept_payment(receiver, sender_address);
        let (_, start_time, _) = get_payment(sender_address, receiver_address);
        assert!(start_time == now, 0);
        timestamp::fast_forward_seconds(period_in_seconds / 2);
        claim_payment(receiver, sender_address);
    ...
    #[expected_failure(abort_code = 4, location = base_module::pay_stream)]
    public entry fun test_accept_as_sender(aptos_framework: &signer, sender: &signer) {
        ...
        let now = 1;
        timestamp::fast_forward_seconds(now);
        accept_payment(sender, sender_address);
    ...
    public entry fun test_claim_as_sender(aptos_framework: &signer, sender: &signer, receiver: &signer) {
        ...
        timestamp::fast_forward_seconds(now);
        accept_payment(receiver, sender_address);
        claim_payment(sender, sender_address);
